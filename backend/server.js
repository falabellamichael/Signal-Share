...
app.post("/api/activity/report", async (req, res) => {
  ...
  try {
    ...
    if (error) {
      console.error("[Bridge] Arcade activity sync error:", error.message);
      return res.status(500).json({ ok: false });
    }

    lastSupabaseSyncKey = `activity|${activity.title}|${Date.now()}`;
    return res.json({ ok: true });
  } catch (error) {
    console.error("[Bridge] Arcade activity sync failed:", error);
    return res.status(500).json({ ok: false });
  }
});
