...
function validateSourceTarget(activeSource, nativeSnapshot, desktopSnapshot) {
  ...
  // CRITICAL FIX #5: In Media Toggle mode, always allow actions to send to bridge even without matching snapshot
  const isMediaToggleMode = activeSource === 'youtube' || activeSource === 'spotify';
  
  if (isMediaToggleMode) {
    console.log(`[Hero] validateSourceTarget: No active snapshot for ${activeSource}, but allowing bridge routing`);
    return { valid: true, target: "system", source, reason: "no-active-source-bridge-routing" };
  }
