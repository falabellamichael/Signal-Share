...
create table if not exists public.user_bans (
  banned_id uuid primary key references auth.users (id) on delete cascade,
  banned_by uuid references auth.users (id) on delete set null,
  reason text,
  created_at timestamptz not null default now(),
  check (banned_by is null or banned_by <> banned_id)
);
...
create index if not exists user_bans_created_idx
...
create or replace function public.is_signal_share_banned(target_user_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where banned_id = target_user_id
  );
$$;

create or replace function public.can_use_direct_messaging(left_user_id uuid, right_user_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select
    ...
    and not public.is_signal_share_banned(left_user_id)
    and not public.is_signal_share_banned(right_user_id)
    and not exists (
      select 1
...
grant select, insert, delete on table public.direct_threads to authenticated;
grant select, insert on table public.messages to authenticated;
grant select, insert, delete on table public.user_blocks to authenticated;
grant select, insert, delete on table public.user_bans to authenticated;
...
revoke all on function public.update_signal_share_oauth_provider(text, jsonb) from public, anon, authenticated;
...
grant execute on function public.update_signal_share_oauth_provider(text, jsonb) to service_role;
...
grant execute on function public.is_signal_share_banned(uuid) to anon, authenticated;
grant execute on function public.can_use_direct_messaging(uuid, uuid) to authenticated;
...
with check (
  (select auth.uid()) = id
  and not public.is_signal_share_banned((select auth.uid()))
);
...
using (
  ...
  and public.can_use_direct_messaging(user_one_id, user_two_id)
);
...
drop policy if exists "participants can delete direct threads" on public.direct_threads;
create policy "participants can delete direct threads"
on public.direct_threads
for delete
to authenticated
using (
...
drop policy if exists "admins and banned users can read bans" on public.user_bans;
create policy "admins and banned users can read bans"
on public.user_bans
for select
to authenticated
using (
...
to authenticated
using (
  not public.is_signal_share_banned((select auth.uid()))
  and
  exists (
    select 1
    ...
    where public.direct_threads.id = public.messages.thread_id
      ...
      and public.can_use_direct_messaging(
        public.direct_threads.user_one_id,
        public.direct_threads.user_two_id
      )
  )
);
